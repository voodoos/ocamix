...
let lwd_table_concat t1 t2 =
  let rec aux = function
    | Some row ->
        let set = Lwd_table.get row in
        ignore @@ Lwd_table.append ?set t1;
        aux @@ Lwd_table.next row
    | None -> ()
  in
  aux @@ Lwd_table.first t2;
  t1
...
let make (type data) ~(ui_table : Schema.fixed_row_height)
  ...
  let table_rows : data row_data Lwd_seq.t Lwd.t =
    Lwd_table.map_reduce
      (fun source_row _data ->
        Lwd_seq.element
          (* TODO: to make the list virtual content should be none by default *)
          { content = Lwd_table.get source_row; source_row; render })
      Lwd_seq.monoid source_rows
  in
    ...
    | Some data ->
        let rendered_row =
          Lwd.map render ~f:(fun render ->
              Lwd_seq.of_list
                (List.map (render 0 data) ~f:(fun elt -> Elwd.div [ elt ])))
        in
        ( 0,
          Lwd_seq.element
